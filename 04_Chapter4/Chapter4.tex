...
\item support for 3D file formats -- importing objects and creating new tools using 3D files such as \lstinline{STL}, \lstinline{STEP} and \lstinline{IGES},
...
A RoboDK project containing a robotic arm, robotic arm tools, additional CAD files, robotic arm frames, and a robotic arm program is called a station. A RoboDK station is saved as one file (\lstinline{.rdk} extension).  The FANUC M-20iA/35M model from the RoboDK robotic arm library is used in this study because it is readily available in the RoboDK library and differs from the FANUC M-20iA/20M robotic arm used in the LSP station at HiLASE only in payload capacity. Robot files in RoboDK have a \lstinline{.robot} extension \cite{robodkstation}.
...
\begin{itemize}
    \item \lstinline{robolink} module, 
    \item \lstinline{item} module, 
    \item \lstinline{robodk} module.
\end{itemize}
...
\begin{itemize}
    \item \lstinline{AutoSetup} -- the user selects the geometry (curves and points) and the model, the curves are automatically transferred to RoboDK,
    \item \lstinline{LoadPart} -- only loads the 3D model from SolidWorks to RoboDK without importing the curves or points,
    \item \lstinline{LoadPoints} -- loads points to RoboDK as a new object. Selected surfaces are used to calculate the curve normals, 
    \item \lstinline{LoadCurves} --  loads curves selected in RoboDK as a new object. Selected surfaces are used to calculate curve normals, 
    \item \lstinline{Settings} -- opens the default settings window \cite{robodksolidworks}.
\end{itemize}
...
\item \textbf{Opening an existing RoboDK station} - Using the \lstinline{Settings} control element of the SolidWorks RoboDK Add-in and then selecting the current project with the \lstinline{Load Project...} button. The next step is to click the \lstinline{LoadCurves} button and highlight the curves that define the robotic arm path and the adjacent surfaces of these curves. Lastly, the action is confirmed by pressing the \lstinline{checkmark} button. The curve is then opened in the selected project. 

\item \textbf{Creating a new RoboDK station} - In this case, the robot path is selected the same way as described in the previous point. A new RoboDK station containing the robotic arm path is opened after pressing the \lstinline{checkmark} button.
...
A post processor in RoboDK is a Python script (\lstinline{.py} extension). All the RoboDK post processors are located in the
\lstinline{C:/RoboDK/Posts/} folder (in case of the Windows operating system).  To use a post processor it must be placed in the \lstinline{C:/RoboDK/Posts/} folder. The user can modify an existing post processor or create a new one from scratch. Modifications of post processors are described in \hyperref[chap:implementation]{Chapter 4} in more detail \cite{robodkposts}. 
